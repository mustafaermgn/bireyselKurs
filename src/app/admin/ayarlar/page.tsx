...
export default function Ayarlar() {
  ...
  const handleHeroImagesUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      updateData({ ...ayarlar, heroImages: [...currentImages, ...newUrls] });
      
      setSuccessMessage(`${files.length} adet fotoğraf başarıyla eklendi!`);
            ...
            <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', marginTop: '15px' }}>
              ...
              <div>
                <div style={{ position: 'relative', display: 'inline-block', marginBottom: '10px' }}>
                  <button style={{ background: '#1e293b', color: 'white', padding: '8px 15px', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '14px' }}>
                    Logo Yükle
                  </button>
                  <input 
                    type="file" 
                    ...
                    onChange={(e) => {
                      if (e.target.files && e.target.files.length > 0) {
                        updateData({ ...ayarlar, logo: URL.createObjectURL(e.target.files[0]) });
